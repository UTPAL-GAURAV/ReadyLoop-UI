...
export function QuestionAttemptRow({ question, index }: { question: QuestionAttempt; index: number }) {
  ...
  const longAnswer = question.userAnswer?.length > 200
  ...
  return (
    <div className="px-4 py-4 space-y-3">
      ...
      {question.interviewerExpectation && (
        <div className="space-y-1">
          <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">What the interviewer wanted to hear</p>
          <p className="text-sm text-muted-foreground italic leading-relaxed">{question.interviewerExpectation}</p>
        </div>
      )}
    </div>
  )
